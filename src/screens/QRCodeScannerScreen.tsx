...
import {insertAccount} from '../database/db'; // Importamos la función para guardar
...
function QRCodeScannerScreen({navigation}: any) {
  ...
  const hasScanned = useRef(false); // Para evitar múltiples escaneos
  ...
  const codeScanner = useCodeScanner({
    codeTypes: ['qr'],
    onCodeScanned: (codes) => {
      if (!isScanning || hasScanned.current) return; // Ignorar si ya se escaneó un QR
      setIsScanning(false);
      hasScanned.current = true; // Marcar que ya se escaneó un QR

      const qrContent = codes[0].value;
      const parsedData = parseQRCode(qrContent ? qrContent : ''); // Intentamos parsear el QR

      if (parsedData) {
        const {email, secret, issuer} = parsedData;
        insertAccount(email, secret, issuer) // Guardamos en la BD
          .then((response: any) => {
            Toast.show({
              type: 'success',
              text1: 'Cuenta guardada',
              text2: 'El QR fue escaneado correctamente.',
              position: 'bottom',
            });
          ...
          .finally(() => {
            setIsScanning(true); // Reanudar escaneo si es necesario
            hasScanned.current = false; // Resetear para futuros escaneos
          });
      } else {
  ...
  const parseQRCode = (content: string) => {
    ...
    if (!match) return null; // No es un QR válido

    const email = decodeURIComponent(match[1]);
    const secret = match[2];
    const issuer = match[3];

    if (!email || !secret || !issuer) return null; // Falta algún parámetro

    return {email, secret, issuer};
  };
  ...
  return (
    <View style={styles.container}>
      <Camera
        ...
        device={device}
        isActive={isScanning} // Controlamos la cámara según el estado
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => {}}>
          <Icon name="checkmark" size={24} color="#f1f1f1" />
        </TouchableOpacity>
      </View>
